  ...
  return (
    <div className={cn("relative", isOpen ? "z-[60]" : "", className)} ref={containerRef}>
      <button
        ...
        className={cn(
          "w-full flex items-center justify-between text-left bg-zinc-950 border border-zinc-800 rounded px-3 h-10 text-xs text-zinc-300 focus:ring-1 focus:ring-blue-600 focus:border-blue-600 outline-none transition-all hover:bg-zinc-900 group font-medium",
          isOpen && "ring-1 ring-blue-600 border-blue-600 bg-zinc-900"
        )}
      >
        ...
        <ChevronDown 
          className={cn(
            "w-4 h-4 ml-2 shrink-0 text-zinc-500 transition-transform duration-200 group-hover:text-zinc-300", 
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={cn(
              "absolute left-0 w-full bg-zinc-950 border border-zinc-800 rounded-lg shadow-2xl overflow-hidden py-1 z-50 min-w-max",
