...
export function Sidebar({ currentTab, onTabChange, isOpen, setIsOpen }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed md:static inset-y-0 left-0 w-64 border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex flex-col flex-shrink-0 z-[90] transition-transform duration-300 md:translate-x-0 h-[100dvh]",
        isOpen ? "translate-x-0" : "-translate-x-full"
      )}>
        <div className="p-4 md:p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
          <div>
            <h1 className="text-lg md:text-xl font-semibold tracking-tight text-zinc-900 dark:text-white flex items-center gap-2">
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/30 shrink-0 hidden md:block">
